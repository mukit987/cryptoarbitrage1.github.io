...
def reformated_orderbook(prices, c_direction):
     ...
     if c_direction == "base_to_quote":
         for i in range(0, len(prices["asks"]), 2):
             ask_price = float(prices["asks"][i])  # Convert to float
             adj_price = 1 / ask_price if ask_price != 0 else 0
             adj_quantity = float(prices["asks"][i + 1]) * ask_price  # Convert to float
             price_list_main.append([adj_price, adj_quantity])
     if c_direction == "quote_to_base":
         for i in range(0, len(prices["bids"]), 2):
             bid_price = float(prices["bids"][i])  # Convert to float
             adj_price = bid_price if bid_price != 0 else 0
             adj_quantity = float(prices["bids"][i + 1])  # Convert to float
             price_list_main.append([adj_price, adj_quantity])
...
def create_html_structure(filename):
    with open(filename, mode='w') as file:
        ...
        <div class="container">
            <h2 class="text-center my-4">Real-Time Arbitrage Results</h2>
            <table class="table table-striped table-hover">
                <thead class="thead-dark">
                    <tr>
                        <th onclick="sortTable(0)">Profit/Loss <i class="fas fa-sort"></i></th>
                        <th onclick="sortTable(1)">Real Rate % <i class="fas fa-sort"></i></th>
                        <th>Contract 1</th>
                        <th>Contract 2</th>
                        <th>Contract 3</th>
                        <th>Direction 1</th>
...
def append_to_html(result, execution_time, start_time_str, end_time_str, filename):
    with open(filename, mode='r+') as file:
        content = file.read()
        file.seek(0)

        # Determine the final currency based on the direction of the last trade (contract_3)
        if result['contract_3_direction'] == 'base_to_quote':
            final_currency = result['contract_3'].split('_')[1]  # quote currency
        else:
            final_currency = result['contract_3'].split('_')[0]  # base currency

        profit_loss = result['profit_loss']
        profit_loss_perc = result['real_rate_perc']

        # Calculate initial amount
        initial_amount = profit_loss / (profit_loss_perc / 100)

        new_row = f"""
            <tr>
                <td>{profit_loss:.6f}</td>
                <td>{profit_loss_perc:.6f}%</td>
                <td>{result['contract_1']}</td>
                <td>{result['contract_2']}</td>
                <td>{result['contract_3']}</td>
                <td>{result['contract_1_direction']}</td>
                <td>{result['contract_2_direction']}</td>
                <td>{result['contract_3_direction']}</td>
            ...
            <tr>
                <td colspan="11">
                    <div class="alert alert-info" role="alert">
                        Trade Analysis: This trade resulted in a {"profit" if profit_loss > 0 else "loss"} of {profit_loss:.6f} {final_currency}, 
                        which is a {profit_loss_perc:.6f}% {"gain" if profit_loss_perc > 0 else "loss"} relative to the initial amount spent.
                        The initial trade amount was approximately {initial_amount:.6f} {final_currency}.
                    </div>
                </td>
            </tr>
        """
        # Insert the new row at the beginning of the <tbody>
        updated_content = content.replace("<tbody>", f"<tbody>\n{new_row}")
        file.write(updated_content)

def finalize_html_structure(filename):
    with open(filename, mode='a') as file:
        ...
        <script>
            /* JavaScript code for sorting and charts */
        </script>
        </body>
        </html>
        """)